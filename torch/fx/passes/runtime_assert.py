...
def insert_deferred_runtime_asserts(
    ...
    def _sympy_interp(expr_to_proxy: dict[Any, Any], expr: Any) -> Any:
    ...
    def add_runtime_asserts(ras: list[Any]) -> None:
        for ra in ras:
            if (
                # redundant
    ...
    for i, node in enumerate(nodes[:-1]):
        ...
        with graph.inserting_before(
            ...
            if (
                ...
                def match_symbol(symint: Any, cb: Any) -> None:
                    if (
            ...
            if unbacked_bindings := resolve_unbacked_bindings(
                ...
                for s, keypath in unbacked_bindings.items():
                    ...
                    def go(node: Any, keypath: Any) -> Any:
                        if keypath == ():
                            return node
                        if (
                            len(keypath) >= 2
