...
class _RegionScooper:
    ...
    def scoop_regions(gm: torch.fx.GraphModule) -> torch.fx.GraphModule:
        ...
        def _is_in_region(region_nodes: set[torch.fx.Node]) -> Any:
            def is_node_supported(_submodules: Any, node: torch.fx.Node) -> bool:
...
def regional_inductor(
    gm: torch.fx.GraphModule, *example_args: Any
) -> torch.fx.GraphModule:
    """
