...
class GraphLowering(torch.fx.Interpreter):
    ...
    def codegen_with_cpp_wrapper(
        ...
        if any(device in self.device_types for device in ["cuda", "xpu"]):
            ...
            if config.triton.autotune_at_compile_time:
                ...
                if config.dump_python_module and self.aot_mode and not self.is_const_graph:
    ...
    def _dump_python_module(self) -> None:
        ...
        In single-pass AOT mode (autotune_at_compile_time=True), only a C++
        wrapper is produced. This method performs an extra codegen pass with
        a Python wrapper to write a .py file whose kernel names match those
        in the C++ binary (and thus the Kineto trace).

        To achieve name parity, the codegen steps are split so that the
        scheduler sees cpp_wrapper=True (matching the real C++ pass's fusion
        decisions) while the wrapper generates Python output.
        """
        ...
        if self.const_module:
            pos = next(self.const_module.wrapper_code._names_iter)
            saved_names_iter = itertools.count(pos)
            self.const_module.wrapper_code._names_iter = itertools.count(pos)
        ...
        # decisions (e.g., can_fuse_mix_order_reduction) match the
        # real C++ pass.
        self.cpp_wrapper = True
        self._update_scheduler()

        # Step 3: Generate code through the Python wrapper. The
        # scheduler drives codegen via wrapper methods and does not
        # check cpp_wrapper during code generation, only during fusion.
        self.cpp_wrapper = False
        with config.patch({"triton.autotune_at_compile_time": False}):
        ...
        # Restore state for the real C++ codegen pass.
        self.cpp_wrapper = True
        if saved_names_iter is not None:
            self.const_module.wrapper_code._names_iter = saved_names_iter
        self.removed_buffers.clear()
        self.removed_operations.clear()
        self.inplaced_to_remove.clear()
        V.graph.sizevars.precomputed_replacements.clear()
