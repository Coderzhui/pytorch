...
from torch._VF import _unpack_dual as _unpack_dual_impl  # type: ignore[attr-defined]
...
def _unwrap_for_grad(
    tensor: torch.Tensor, level: int
) -> torch.Tensor:
    """
    Thin wrapper around torch._C._functorch._unwrap_for_grad that is used
    to proxy in PT2 export/compile fx graph for forward-mode AD.
    """
    from torch._export.utils import _maybe_find_pre_dispatch_tf_mode_for_export
